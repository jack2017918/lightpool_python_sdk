...
class LightPoolClient:
    ...
    async def submit_transaction(
        self, transaction: "VerifiedTransaction"
    ) -> Dict[str, Any]:
        """
        提交交易
        ...
        for action in transaction.signed_transaction.transaction.actions:
            action_dict = {
                "inputs": [
                    objectid_to_bytes(obj_id) for obj_id in action.input_objects
                ],
                "contract": address_to_bytes(action.target_address),
                "action": self._action_name_to_u64(action.action_name),
                "params": list(
                    action.params
                ),  # 字节数组转换为整数列表，与Rust Vec<u8>兼容
            }
            actions_list.append(action_dict)

        # 构造transaction对象，与Rust Transaction结构保持一致
        transaction_dict = {
            "sender": address_to_bytes(
                transaction.signed_transaction.transaction.sender
            ),
        ...
        result = await self._make_request(
            "submitTransaction", submit_transaction_params
        )
        ...
        return {
            ...
            "receipt": TransactionReceipt(
                status=ExecutionStatus(
                    result.get("receipt", {}).get("status", "failure")
                ),
    ...
    async def get_transaction_receipt(
        ...
        try:
            result = await self._make_request(
                "getTransactionReceipt", {"digest": digest}
            )
    ...
    async def get_balance(
        self, address: Address, object_id: ObjectID
        ...
        try:
            result = await self._make_request(
                "getBalance", {"address": str(address), "objectId": str(object_id)}
            )
            return result
        except RpcError:
            return None
    ...
    async def get_market_info(self, market_id: ObjectID) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = await self._make_request(
                "getMarketInfo", {"marketId": str(market_id)}
            )
            return result
        except RpcError:
            return None
    ...
    async def get_order_book(
        self, market_id: ObjectID, depth: int = 10
    ) -> Optional[Dict[str, Any]]:
        """
        获取订单簿
        ...
        try:
            result = await self._make_request(
                "getOrderBook", {"marketId": str(market_id), "depth": depth}
            )
            return result
        except RpcError:
            return None
    ...
    async def get_orders(
        self, address: Address, market_id: Optional[ObjectID] = None
    ) -> List[Dict[str, Any]]:
        """
        获取用户订单
    ...
    async def get_trades(
        self, market_id: ObjectID, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        获取交易历史
        ...
        try:
            result = await self._make_request(
                "getTrades", {"marketId": str(market_id), "limit": limit}
            )
    ...
    async def get_account_info(self, address: Address) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = await self._make_request(
                "getAccountInfo", {"address": str(address)}
            )
            return result
        except RpcError:
            return None
